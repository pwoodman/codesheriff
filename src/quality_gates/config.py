...
from quality_gates.config_schema import (
    CONFIG_VERSION,
    DEFAULT_EXCLUDE,
    DEFAULT_FAIL_ON,
...
class QualityConfig:
    ...
    review_rules_dir: str = ".sheriff/rules"
    ...
    policy: str = "adopt"
    policy_baseline: str = ".sheriff-baseline.json"
...
def _normalize_brand_keys(data: dict[str, Any]) -> dict[str, Any]:
    """Accept the v2 ``[sheriff]`` key and the legacy ``[quality]`` key alike."""
    if "sheriff" not in data:
        return data
    merged = dict(data)
    legacy = merged.pop("sheriff")
    base = merged.get("quality")
    if isinstance(base, dict) and isinstance(legacy, dict):
        combined = dict(legacy)
        combined.update(base)
        merged["quality"] = combined
    else:
        merged["quality"] = legacy
    return merged
...
def config_path(project: Path) -> Path:
    """Config file for the project, preferring the current name over the legacy one."""
    preferred = project / "sheriff.toml"
    legacy = project / "quality.toml"
...
def load_config(project: Path) -> QualityConfig:
    ...
    data = _normalize_brand_keys(data)
    ...
    return QualityConfig(
        ...
        review_rules_dir=str(review.get("rules_dir", ".sheriff/rules")),
        ...
        policy=policy,
        policy_baseline=str(quality.get("baseline") or ".sheriff-baseline.json"),
...
def _apply_trusted_merge_policy(project: Path, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if verify.returncode != 0:
            return data
        raw = None
        for name in ("sheriff.toml", "quality.toml"):
        ...
        return data
    if raw is None or raw.returncode:
        return data
    try:
